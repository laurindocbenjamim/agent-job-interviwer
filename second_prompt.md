...
add in both sides the time the interview has started and end. When user clicks on stop interview it is not stopping the agent, it still speechs even in Interview Complete mode. Fix this issue. Also the button to print the interview pdf report in the admin page was not added. The pdf report must be clear explainable with the information generated by another specialist agent that analyse the CV results, time, questions, answers, the rules defined by manager. The pdf must be well formatted with header with the app name, company name, candidate name, date and init and end time. The pdf should not have special characaters like "## and **"